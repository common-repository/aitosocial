...
use FSP\ParagonIE\ConstantTime\Hex;
use FSP\phpseclib3\Common\Functions\Strings;
use FSP\phpseclib3\Crypt\EC\BaseCurves\Base as BaseCurve;
use FSP\phpseclib3\Crypt\EC\BaseCurves\Binary as BinaryCurve;
use FSP\phpseclib3\Crypt\EC\BaseCurves\Prime as PrimeCurve;
...
trait Common
{
    ...
    protected static function loadCurveByParam(array $params)
    {
        ...
        if (isset($params['namedCurve'])) {
            $curve = '\\FSP\\phpseclib3\\Crypt\\EC\\Curves\\' . $params['namedCurve'];
    ...
    private static function encodeParameters(BaseCurve $curve, $returnArray = \false, array $options = [])
    {
        ...
        if ($useNamedCurves) {
            ...
            foreach (new \DirectoryIterator(__DIR__ . '/../../Curves/') as $file) {
                ...
                $class = 'FSP\\phpseclib3\\Crypt\\EC\\Curves\\' . $testName;

 ...
 */
namespace FSP\phpseclib3\Crypt\EC\Formats\Keys;

use FSP\phpseclib3\Common\Functions\Strings;
use FSP\phpseclib3\Crypt\Common\Formats\Keys\PKCS8 as Progenitor;
use FSP\phpseclib3\Crypt\EC\BaseCurves\Base as BaseCurve;
...
use FSP\phpseclib3\File\ASN1\Maps;
...
abstract class PKCS8 extends Progenitor
{
    ...
    public static function load($key, $password = '')
    {
        ...
        if (!$params) {
            throw new \RuntimeException('FSP\\Unable to decode the parameters using Maps\\ECParameters');
        }
        $components = [];
    ...
    public static function savePrivateKey(BigInteger $privateKey, BaseCurve $curve, array $publicKey, $password = '', array $options = [])
    {
        ...
        if ($curve instanceof TwistedEdwardsCurve) {
            return self::wrapPrivateKey("\x04 " . $privateKey->secret, [], null, $password, $curve instanceof Ed25519 ? 'id-Ed25519' : 'id-Ed448');
        }

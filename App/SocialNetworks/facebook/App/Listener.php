	...
	*/
	...
	private static function cookieMethod( $postData )
	{
		...
		if ( empty( $fbDriver->getDTSG() ) )
		{
			...
			AccountService::disable_account( $accID, fsp__( 'The account is disconnected from the AItoSocial plugin. Please add your account to the plugin without deleting the account from the plugin; as a result, account settings will remain as it is.' ) );

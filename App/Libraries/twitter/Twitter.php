...
class Twitter extends SocialNetwork
{
	...
	public static function sendPost ( $appId, $type, $message, $comment, $username, $link, $images, $video, $accessToken, $accessTokenSecret, $proxy )
	{
		...
		if ( ! empty( $result[ 'errors' ] ) && is_array( $result[ 'errors' ] ) )
		{
			if ( isset( $result[ 'errors' ][ 0 ][ 'code' ] ) && $result[ 'errors' ][ 0 ][ 'code' ] == 185 )
			{
                ...
                {
                    $errorMsg = fsp__( 'The Standard APP has reached the hourly limit for sharing posts. The limit is assigned by Twitter and you either need to <a href="https://project.puredevs.com/documentation/fs-poster-schedule-share-wordpress-posts-to-twitter-automatically" target="_blank">create a Twitter App</a> for your own use or use the <a href="https://project.puredevs.com/documentation/fs-poster-schedule-share-wordpress-posts-to-twitter-automatically" target="_blank">Cookie method</a> to add your account to the plugin.', [], FALSE );
                }
                else
                {
                    $errorMsg = fsp__( 'The AItoSocial Standard App should be used only for testing purposes, please create your personal App to share posts on Twitter. <a href="https://aitosocials.com/documentation/fs-poster-schedule-share-wordpress-posts-to-twitter-automatically">How?</a>' );
                }
			}
		...
		else if ( isset( $result[ 'status' ] ) && $result[ 'status' ] == 429 && !empty( $appInfo[ 'slug' ] ) )
		{
			return [
				'status'    => 'error',
				'error_msg' => fsp__( 'The AItoSocial Standard App should be used only for testing purposes, please create your personal App to share posts on Twitter. <a href="https://aitosocials.com/documentation/fs-poster-schedule-share-wordpress-posts-to-twitter-automatically">How?</a>' )
			];
		}
	...
	public static function getLoginURL ( $appId )
	{
		...
		if ( empty( $tokens[ 'oauth_token' ] ) || empty( $tokens[ 'oauth_token_secret' ] ) )
		{
			...
			{
				self::error( fsp__( 'You need to apply for Elevated access via the Developer Portal to share posts. <a href="https://project.puredevs.com/documentation/fs-poster-schedule-share-wordpress-posts-to-twitter-automatically">How to?</a>', [], FALSE ), FALSE );
			}
	...
	public static function authorize ( $appInf, $oauth_token, $oauth_token_secret, $proxy )
	{
		...
		if ( isset( $user[ 'errors' ][ 0 ][ 'code' ] ) && $user[ 'errors' ][ 0 ][ 'code' ] == 453 )
		{
			return [
				'status'    => FALSE,
				'error_msg' => fsp__( 'You need to apply for Elevated access via the Developer Portal to share posts. <a href="https://project.puredevs.com/documentation/fs-poster-schedule-share-wordpress-posts-to-twitter-automatically">How to?</a>', [], FALSE ),
				'esc_html'  => FALSE
			];
		}

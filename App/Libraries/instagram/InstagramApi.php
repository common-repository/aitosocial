...
class InstagramApi
{
	...
	public static function sendPost ( $accountInfo, $type, $message, $comment, $link, $images, $videoPath, $videoURL, $instagramPinThePost = 0 )
	{
		if ( $type == 'image' )
		{
			if ( ! extension_loaded( 'gd' ) )
			{
				return [
					'status'    => 'error',
					'error_msg' => fsp__( 'You should install and enable GD, libjpeg, libpng, and freetype libraries. <a href=\'https://project.puredevs.com/documentation/commonly-encountered-issues#issue3\' target=\'_blank\'>Learn more.</a>', [], FALSE )
				];
			}
	...
	public static function checkAccount ( $accountInfo )
	{
		...
		else if ( $password === '*****' )
		{
			...
			if ( ! $instagram_res )
			{
				$result = [
					'error'     => TRUE,
					'error_msg' => fsp__( 'The account is disconnected from the AItoSocial plugin. Please update your account cookie to connect it to the plugin again. <a href=\'https://aitosocials.com/documentation/commonly-encountered-issues#issue8\' target=\'_blank\'>How to?</a>', [], FALSE )
				];
			}
		}
		else
	...
	private static function renderVideo ( $video_path, $target )
	{
		...
		{
			throw new Exception( fsp__( 'exec() function have to be enabled to share videos on Instagram. <a href=\'https://project.puredevs.com/documentation/how-to-install-ffmpeg\' target=\'_blank\'>How to?</a>', [], FALSE ) );
		}
	...
	public static function error ( $error_msg = NULL, $account_id = NULL )
	{
		...
		if ( isset( $error_msg ) && ! empty( $error_msg ) )
		{
			...
			{
				AccountService::disable_account( $account_id, fsp__( 'The account is disconnected from the plugin. Please add your account to the plugin again by getting the cookie on the browser <a href=\'https://project.puredevs.com/documentation/fs-poster-schedule-auto-publish-wordpress-posts-to-instagram\' target=\'_blank\'>Incognito mode</a>. And close the browser without logging out from the account.', [], FALSE ) );

				return fsp__( 'The account is disconnected from the plugin. Please add your account to the plugin again by getting the cookie on the browser <a href=\'https://project.puredevs.com/documentation/fs-poster-schedule-auto-publish-wordpress-posts-to-instagram\' target=\'_blank\'>Incognito mode</a>. And close the browser without logging out from the account.', [], FALSE );
			}
			else
			{

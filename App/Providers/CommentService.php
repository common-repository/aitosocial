...
class CommentService
{
			...
			if ( ! empty ( $comments ) )
			{
				...
				foreach ( $comments as $comment )
				{
					...
					self::insertComment(
						...
						empty( $comment[ 'created_time' ] ) ? Date::dateSQL() : $comment[ 'created_time' ]
					);
				}
	...
	private static function getPosts ()
	{
		...
		$postTypesIn = DB::DB()->prepare( implode( ',', array_fill( 0, count( $postTypes ), '%d' ) ), $postTypes );
		$nodesIn     = DB::DB()->prepare( implode( ',', array_fill( 0, count( $fbNodes ), '%d' ) ), $fbNodes );

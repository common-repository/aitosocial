...
(function ($) {
	...
	doc.ready( function () {
		...
		$( '#fspReports_clicksTypes2' ).on( 'select2:select', function () {
			let _this = $( this );
			let type = _this.val();

			if ( type === lastSelectedClicksType2 )
			{
				return;
			}

			lastSelectedClicksType2 = type;
			...
			FSPoster.ajax( 'report1_data', { type }, function (result) {
				...
				let ctx = document.getElementById( 'fspReports_clicksChart2' ).getContext( '2d' );
				chart_1 = new Chart( ctx, {
					type: 'line',
					data: {
						labels: result[ 'labels' ],

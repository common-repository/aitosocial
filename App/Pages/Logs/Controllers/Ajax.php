...
trait Ajax
{
	...
	public function report3_data ()
	{
		...
		foreach ( $feeds as $feed )
		{
			...
			$wp_post_link = ! ( $postInf->post_type == 'fs_post' || $postInf->post_type == 'fs_post_tmp' ) ? ( site_url() . '/?p=' . $feed[ 'post_id' ] ) : ( trim( get_admin_url(), '/' ) . '/admin.php?page=ai-poster-share&post_id=' . $feed[ 'post_id' ] );
	...
	public function export_logs_to_csv ()
	{
		...
		$delimiter = ',';
		$filename  = 'AItoSocial_logs_' . date( 'Y-m-d' ) . '.csv';
		$fields    = [
			//fsp__( 'ID' ),
			fsp__( 'Account Name' ),
			fsp__( 'Account Link' ),

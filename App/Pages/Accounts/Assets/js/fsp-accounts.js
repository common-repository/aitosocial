	...
	doc.ready( function () {
		...
		doc.on( 'click', '.fsp-tab[data-component]', function () {
			...
			if ( currentComponent )
			{
				...
				window.history.pushState( {}, '', `?page=ai-poster-accounts&tab=${ currentComponent }&filter_by=${ filter_by }` );
			}
			...
			if ( currentComponent )
			{
				...
				window.history.pushState( {}, '', `?page=ai-poster-accounts&view=groups&group=${ currentComponent }&filter_by=${ filter_by }` );
			}
		} ).on( 'click', function ( e ) {
		...
		$( '#fspMoreMenu > #fsp-export-webhook' ).on( 'click', function () {
			...
			FSPoster.confirm( fsp__('If you want to share this template with other AItoSocial customers, please submit exported JSON via Google Form. After review it will be published in templates list. It is reccomended to redact informations like credentials, tokens, etc. before submit it.'), function (){

...
trait Ajax
{
	...
	public function schedule_get_calendar ()
	{
		...
		// need to hide this to show old schedule
		...
		$getPlannedDays = DB::DB()->get_results( "SELECT * FROM `" . DB::table( 'schedules' ) . "` WHERE `start_date` <= '$lastDate' AND `user_id` = '$myId' AND `blog_id` = '" . Helper::getBlogId() . "'", ARRAY_A );
		...
		foreach ( $getPlannedDays as $planInf )
		{
			...
			while ( $cursorDayTimestamp <= $planEnd )
			{
				...
				$days[] = [
					...
					'status'    => $planInf[ 'status' ],
					'start_date'    => $planInf[ 'start_date' ],
					'next_execute_time'    => $planInf[ 'next_execute_time' ],
					'post_freq'    => $planInf[ 'post_freq' ],
